...
export default function NotificationsTab({ showMsg }: { showMsg: (type: 'success' | 'error', text: string) => void }) {
  const [activeTab, setActiveTab] = useState<'send' | 'logs' | 'recipients'>('send');
  ...
  async function sendManualPush() {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/send-manual-push`,
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        showMsg('success', `${result.sent} notifications envoyées`);
        setPushTitle('');
        setPushBody('');
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs">Envoyer manuellement & tracker l'historique</p>
          </div>
        </div>
      </div>
      ...
      {activeTab === 'send' && (
        <div className="space-y-6">
          ...
          <motion.div
            ...
            className="bg-[#0f0f0f] border border-white/5 rounded-xl p-6 space-y-4"
          >
            ...
            <div>
              ...
              />
              <p className="text-[9px] text-gray-600 mt-1">{pushTitle.length}/100</p>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-[9px] text-gray-600 mt-1">{pushBody.length}/240</p>
            </div>
            ...
            >
              {isSending ? (
                ...
                <><Send size={16} /> Envoyer à {0} utilisateurs</>
              )}
            </button>
          </motion.div>
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p>Aucun historique de notifications</p>
            </div>
          ) : (
            logs.map(log => (
              <motion.div
                ...
                className="bg-[#0f0f0f] border border-white/5 rounded-xl p-4 cursor-pointer hover:border-blue-500/30 transition-all"
              >
                <div className="flex items-center justify-between">
            ...
            <>
              ...
              {recipientsLoading ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p>Aucun destinataire avec ce statut</p>
                </div>
              ) : (
                  ...
                  {filteredRecipients.map(r => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        {r.error_message && (
                          <p className="text-[10px] text-gray-500 mt-0.5">{r.error_message}</p>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
